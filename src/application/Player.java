...
public class Player{
    ...
    public Player(PlayerII playerI, int characters) {
        super();
        this.lvl = 0;
        for (int k = 0; k < 5; k++) {
        	imageRight[k] = new Image(("images/player/"+characters+".png"));
        	imageLeft[k] = new Image(("images/player/"+characters+"-right.png"));
        }
        this.x = 100;
    ...
    void collision(Box box) {
        ...
        if (Math.abs(y - floor) <= 5 && !falling && !jumping && floor != (int) 
        		(550 - 85 - imageRight[0].getHeight())) {
            floor = (int) (550 - 85 - imageRight[0].getHeight());
            falling = true;
        }
    }
        ...
        if (Math.abs(y - box.getY() - box.getImage().getHeight()) <= 10 && jumping) {
            ...
            if (box.getI() < 46) { // box is hit first time
                ...
                if (rand.nextInt(10) <= 1) {
                    ...
                    BoxCoin boxCoin = new BoxCoin(box.getX() + (int) (box.getImage().getWidth() / 4), 
                    		box.getY());
